...
class PartA2FCHeadTS(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, num_class=1, **kwargs):
        super().__init__(num_class=num_class, model_cfg=model_cfg)
        self.model_cfg = model_cfg

        self.SA_modules = nn.ModuleList()
        block = self.post_act_block_ts

        c0 = self.model_cfg.ROI_AWARE_POOL.NUM_FEATURES // 2
        self.conv_part = nn.Sequential(
            block(4, 64, 3, padding=1),
            block(64, c0, 3, padding=1),
        )
        self.conv_rpn = nn.Sequential(
            block(input_channels, 64, 3, padding=1),
            block(64, c0, 3, padding=1),
        )
    ...
    def post_act_block_ts(self, in_channels, out_channels, kernel_size, indice_key=None, stride=1, padding=0,
                    conv_type='tsconv', norm_fn=None):
        if norm_fn is None:
            norm_fn = spnn.BatchNorm(out_channels, eps=1e-3, momentum=0.01)
    ...
    def forward(self, batch_dict):
        ...
        merged_feature = torch.cat((x_rpn.feats, x_part.feats), dim=1)  # (N, C)
