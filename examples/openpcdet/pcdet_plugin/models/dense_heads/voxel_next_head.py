...
class SeparateHeadTS(nn.Module):
    def __init__(self, input_channels, sep_head_dict, kernel_size, init_bias=-2.19, use_bias=False):
        ...
        for cur_name in self.sep_head_dict:
            ...
            for k in range(num_conv - 1):
                fc_list.append(
                    nn.Sequential(
                        spnn.Conv3d(input_channels, input_channels, kernel_size=(1, kernel_size,kernel_size), padding=int(kernel_size//2), bias=use_bias),
                        spnn.BatchNorm(input_channels),
                        spnn.ReLU(),
                    )
                )
            fc_list.append(
                    spnn.Conv3d(input_channels, output_channels, kernel_size=1, bias=True),
                )
    ...
    def forward(self, x):
        ret_dict = {}
        for cur_name in self.sep_head_dict:
            ret_dict[cur_name] = self.__getattr__(cur_name)(x).feats

        return ret_dict


class VoxelNeXtHeadTS(nn.Module):
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts, voxel_indices, spatial_shape):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            if self.double_flip:
                ...
                voxel_indices_ = voxel_indices[:,:-1]
    ...
    def _get_voxel_infos(self, x):
        spatial_shape = x.spatial_range[1:]  # Might need to include channels
        voxel_indices = x.coords
        # print(x.coords)
        spatial_indices = []
        num_voxels = []
        batch_size = x.spatial_range[0]
